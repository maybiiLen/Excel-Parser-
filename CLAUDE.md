...
- Rendered as a title row (`<p class="ws-title">`) + nested rows (`<p class="ws-lvl" data-level="N">`); `buildWordHtml` maps the title to a Word heading style when `headingStyleName` is set, and the body always to the app's direct per-level look.
...
## Styling
**Title → Word heading mapping:** one shared input — **Heading style** (default `Heading 1`) — is a Word style name. When set, the title carries `mso-style-name:"<heading>"`+`mso-outline-level:1`, so a **Use Destination Styles** paste makes the title adopt the destination document's heading (font/size/color/numbering from the template). Leave it blank to use the app's direct level-1 look. The name is sanitized (`sanitizeStyleName`) before landing in the `mso-style-name` attribute. **The body is never mapped** — the nested rows always use the app's direct per-level look, so the Word output matches the live preview (including per-level bold).
