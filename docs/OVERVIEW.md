...
3. **Render → preview.** The tree is rendered to HTML and shown in each card's live preview (or toggle to inspect the raw Grid as JSON). A per-level **Markers** picker adds `1./a./i.`/etc.; per field you can hide/bold/italic/underline the `Field name:` label, **sort** the groups at that field (↕ off / ↑ asc / ↓ desc, numeric + text aware), and per level add a **blank line after** each group.
